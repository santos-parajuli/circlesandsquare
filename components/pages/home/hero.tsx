			...
			<div className='mx-auto flex h-full max-w-7xl flex-col gap-16 px-4 sm:px-6 lg:px-8'>
				...
				<div className='grid grid-cols-1 gap-6 gap-y-12 md:gap-y-16 lg:grid-cols-5'>
					<div className='flex w-full flex-col justify-center gap-5 max-lg:items-center lg:col-span-3 lg:h-95.5'>
						<h1 className='text-3xl leading-[1.29167] font-semibold text-balance max-lg:text-center sm:text-4xl lg:text-5xl'>Artisan Bakery in Toronto – Circles & Squares</h1>

						<p className='text-muted-foreground max-w-xl text-xl max-lg:text-center'>
							Experience the freshest bread, pastries, and savoury dishes crafted by hand daily. At Circles & Squares Bakery, we combine tradition with creativity to make every bite unforgettable.
						</p>
						...
						<CarouselContent>
							{menudata.map((item) => (
								<CarouselItem key={item.id} className='flex w-full items-center justify-center'>
									<img src={item.img} alt={item.imgAlt} className='obeh size-64 lg:size-95 object-contain' />
								</CarouselItem>
							))}
						</CarouselContent>
					</Carousel>
				...
				<div className=' grid-cols-1 gap-24 gap-y-12 md:gap-y-16 lg:grid-cols-5 grid '>
					<Carousel
						className='relative w-full max-lg:order-2 lg:col-span-3 hidden lg:block'
						setApi={setThumbApi}
						opts={{
							loop: true,
						}}>
						...
						<CarouselContent className='my-1 flex'>
							{menudata.map((item, index) => (
								<CarouselItem key={item.id} className={cn('basis-1/2 cursor-pointer sm:basis-1/3 md:basis-1/4 lg:basis-1/3 xl:basis-1/4')} onClick={() => handleThumbClick(index)}>
					...
					<Carousel
						...
						<CarouselContent>
							{menudata.map((item) => (
								<CarouselItem key={item.id} className='flex h-full min-h-14 w-full items-center justify-center gap-4 px-6 lg:items-center'>
